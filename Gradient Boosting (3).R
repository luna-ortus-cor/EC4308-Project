...
# build expanding-window CV folds (validation indices only)
k <- 5
n <- nrow(X_train)
...
for (i in 1:k) {
  ...
  folds[[i]]  <- valid_start:valid_end  # VALIDATION indices (correct)
}
...
# depths <- c(3, 4, 5, 6)          # allow deeper trees
# min_child <- c(1, 3, 5, 7) # allow smaller leaf size
# subsamps <- c(0.5, 0.6, 0.7, 0.8, 0.9) # allow more variance
# etas <- c(0.02, 0.03, 0.05, 0.07, 0.10) # allow deeper learning
# colsamps <- c(0.3, 0.4, 0.5, 0.6, 0.8)
