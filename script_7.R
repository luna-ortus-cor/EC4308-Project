...
pcr_pred_out <- predict(pcr_model, newdata = data.frame(X_train_scaled = X_test_scaled), ncomp = best_ncomp_pcr)
...
for (strategy in strategies) {
  for (ftype in forecast_types) {
    for (h in horizons) {
      key <- paste0(strategy, "_", ftype, "_h", h)
      if (!key %in% names(all_forecasts)) next
...
# Choose forecast strategy
FORECAST_STRATEGY <- "recursive"  # Options: "static", "recursive", "rolling"
ROLLING_WINDOW_SIZE <- 120  # Only used if strategy = "rolling"

cat("Forecast Strategy:", toupper(FORECAST_STRATEGY), "\n")
if (FORECAST_STRATEGY == "rolling") {
  cat("Rolling Window Size:", ROLLING_WINDOW_SIZE, "observations\n")
}

forecast_results_all <- list()
for (strategy in c("static", "recursive", "rolling")) {
  cat("\n===== STRATEGY:", toupper(strategy), "=====\n")
  ...
  for (h in horizons) {
    ...
    # Generate forecasts based on strategy
    for (i in 1:n_available) {
      
      # ===========================
      # Define window based on strategy
      # ===========================
      if (strategy == "static") {
        # Static: always use original training data
        y_window <- y_train
        X_window <- X_train_scaled
        ...
        if (i == 1) {
          y_window <- tail(y_train, ROLLING_WINDOW_SIZE)
          X_window <- tail(X_train_scaled, ROLLING_WINDOW_SIZE)
        } else {
          n_obs <- i - 1
          y_all <- c(y_train, y_test[1:n_obs])
          X_all <- rbind(X_train_scaled, X_test_scaled[1:n_obs, , drop = FALSE])
          y_window <- tail(y_all, ROLLING_WINDOW_SIZE)
          X_window <- tail(X_all, ROLLING_WINDOW_SIZE)
        }
      }
    ...
    cat(sprintf("MSE - AR: %.4f, ARIMA: %.4f\n", ar_mse, arima_mse))
  ...
  forecast_results_all[[strategy]] <- forecast_results 
}
...
cat("\n=============================\n")
cat("FORECAST VISUALIZATIONS\n")
cat("=============================\n\n")

plot_list_forecasts <- list()
idx <- 1

for (strategy in c("static", "recursive", "rolling")) {
  for (h in horizons) {
    key <- paste0(strategy, "_h", h)
    if (!key %in% names(forecast_series_all)) next
    
    fc <- forecast_series_all[[key]]
    ...
    plot_data <- data.frame(
      ...
      pivot_longer(cols = c(Actual, AR, ARIMA),
                   names_to = "Series",
                   values_to = "Value")
    ...
    p <- ggplot(plot_data, aes(x = Date, y = Value, color = Series)) +
      ...
      scale_color_manual(values = c("Actual" = "black",
                                    ...
                                    "ARIMA" = "red")) +
      theme_minimal() +
      labs(title = paste(toupper(strategy), "- Horizon", h),
           x = "Date", y = "INDPRO Growth (%)") +
      theme(legend.position = "bottom",
            plot.title = element_text(size = 10))
    
    plot_list_forecasts[[idx]] <- p
    idx <- idx + 1
  }
}
cat("Displaying forecast plots for all strategies and horizons...\n")
do.call(grid.arrange, c(plot_list_forecasts, ncol = 4))
...
cat("\n=============================\n")
cat("FORECAST ERROR ANALYSIS\n")
cat("=============================\n\n")

plot_list_errors <- list()
idx <- 1
...
for (strategy in c("static", "recursive", "rolling")) {
  for (h in horizons) {
    ...
    error_data <- data.frame(
      Date = fc$dates,
      AR_Error = fc$ar - fc$actual,
      ARIMA_Error = fc$arima - fc$actual,
      VAR_Error = fc$var - fc$actual
    ) %>%
      pivot_longer(cols = c(AR_Error, ARIMA_Error, VAR_Error),
                   names_to = "Model",
                   values_to = "Error") %>%
      mutate(Model = gsub("_Error", "", Model))
    
    p <- ggplot(error_data, aes(x = Date, y = Error, color = Model)) +
      geom_line(alpha = 0.6) +
      geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
      theme_minimal() +
      labs(title = paste(toupper(strategy), "- h =", h),
           x = "Date", y = "Forecast Error") +
      theme(legend.position = "bottom",
            plot.title = element_text(size = 10))
    
    plot_list_errors[[idx]] <- p
    idx <- idx + 1
  }
}
cat("Displaying forecast error plots...\n")
do.call(grid.arrange, c(plot_list_errors, ncol = 4))
...
# Combine all results
all_results <- do.call(rbind, forecast_results_all)
print(all_results)

# MSE comparison plot - all strategies
mse_plot_data <- all_results %>%
  select(Strategy, Horizon, AR_MSE, ARIMA_MSE, VAR_MSE) %>%
  pivot_longer(cols = c(AR_MSE, ARIMA_MSE, VAR_MSE), 
...
p_mse <- ggplot(mse_plot_data, aes(x = factor(Horizon), y = MSE, fill = Model)) +
  ...
  labs(title = "MSE Comparison: AR vs ARIMA vs VAR",
       subtitle = "Across Different Forecasting Strategies",
       x = "Forecast Horizon",
       y = "Mean Squared Error") +
  theme(legend.position = "bottom")
print(p_mse)

# Relative performance across strategies
all_results$ARIMA_vs_AR <- all_results$ARIMA_MSE / all_results$AR_MSE
all_results$VAR_vs_AR <- all_results$VAR_MSE / all_results$AR_MSE
cat("\n--- Relative Performance by Strategy (ratio < 1 = better than AR) ---\n")
print(all_results[, c("Strategy", "Horizon", "ARIMA_vs_AR", "VAR_vs_AR")])

# Best model by horizon and strategy
best_models <- all_results %>%
  rowwise() %>%
  mutate(Best_Model = c("AR", "ARIMA", "VAR")[which.min(c(AR_MSE, ARIMA_MSE, VAR_MSE))],
         Best_MSE = min(AR_MSE, ARIMA_MSE, VAR_MSE)) %>%
  select(Strategy, Horizon, Best_Model, Best_MSE)
cat("\n--- Best Model by Strategy and Horizon ---\n")
print(best_models)

# Summary plot: which method works best?
p_best <- ggplot(best_models, aes(x = factor(Horizon), y = Best_MSE, fill = Best_Model)) +
  geom_bar(stat = "identity") +
  facet_wrap(~ Strategy, ncol = 3) +
  theme_minimal() +
  labs(title = "Best Performing Model by Strategy and Horizon",
       x = "Forecast Horizon",
       y = "MSE",
       fill = "Best Model") +
  theme(legend.position = "bottom")
