...
# Summary comparison with MSE ----------------------------------------------

comparison <- data.frame(
  Model = c("LASSO-BIC", "LASSO-Rolling-CV (1se)", 
            "Post-LASSO (BIC)", "Post-LASSO (Rolling CV)", "Adaptive LASSO"),
  MSE = round(c(results_bic$rmse^2, results_rolling$rmse^2, 
                results_post_bic$rmse^2, results_post_cv$rmse^2, results_ada$rmse^2), 4),
  MAE = round(c(results_bic$mae, results_rolling$mae, 
                results_post_bic$mae, results_post_cv$mae, results_ada$mae), 4),
  R2 = round(c(results_bic$r2, results_rolling$r2, 
...
# 1. LASSO Models (BIC and Rolling CV)
png("lasso_plot.png", width = 1000, height = 500)  # Or adjust in RStudio with par(pin=c(7,2))
plot(test_data$sasdate, y_test, type = "l", col = "black", lwd = 1,
     xlab = "Date", ylab = "Industrial Production Growth (%)",
     main = "Rolling Window LASSO Out-of-Sample Forecasts")
lines(test_data$sasdate, pred_test_bic, col = "red", lwd = 1, lty = 1)
lines(test_data$sasdate, pred_test_rolling, col = "blue", lwd = 1, lty = 2)
legend("topleft", legend = c("Actual", "LASSO-BIC", "LASSO-Rolling-CV"),
       col = c("black", "red", "blue"), lty = c(1,1,2), lwd = 1, bty = "n")
dev.off()

# 2. Post-LASSO Models (BIC and Rolling CV)
png("your_plot.png", width = 1000, height = 500)
# Then your plot code
plot(test_data$sasdate, y_test, type = "l", col = "black", lwd = 1,
     xlab = "Date", ylab = "Industrial Production Growth (%)",
     main = "Rolling Window Post-LASSO Out-of-Sample Forecasts")
lines(test_data$sasdate, pred_test_post_bic, col = "red", lwd = 1)
lines(test_data$sasdate, pred_test_post_cv, col = "blue", lwd = 1, lty = 2)
legend("topleft", legend = c("Actual", "Post-LASSO BIC", "Post-LASSO Rolling CV"), 
       col = c("black", "red", "blue"), lty = c(1,1,2), lwd = 1, bty = "n")
dev.off()
...
# 3. Adaptive LASSO
png("ada_lasso_plot.png", width = 1000, height = 500)  # Or use par(pin=c(7,2)) before plot in RStudio
plot(test_data$sasdate, y_test, type = "l", col = "black", lwd = 1,
     xlab = "Date", ylab = "Industrial Production Growth (%)",
     main = "Rolling Window Adaptive LASSO Out-of-Sample Forecasts")
lines(test_data$sasdate, pred_test_ada, col = "orange", lwd = 1, lty = 1)
legend("topleft", legend = c("Actual", "Adaptive LASSO"),
       col = c("black", "orange"), lty = c(1,1), lwd = 1, bty = "n")
dev.off()
